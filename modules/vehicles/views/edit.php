<?php
$pageTitle = 'Editează Vehicul';
$errors = $_SESSION['errors'] ?? [];
$formData = $_SESSION['form_data'] ?? $vehicle;
unset($_SESSION['errors'], $_SESSION['form_data']);
?>
<div class="container-fluid">
  <h1 class="h3 mb-4"><i class="fas fa-edit"></i> Editează Vehicul: <?= htmlspecialchars($vehicle['registration_number']) ?></h1>
  <?php if (!empty($errors)): ?>
  ...
  <div class="card shadow">
    <div class="card-body">
      <form method="post">
        ...
        <div class="row">
          ...
          <div class="col-md-4 mb-3">
            ...
            <select class="form-control" name="fuel_type" required>
              <option value="petrol" <?= ($formData['fuel_type'] ?? '') == 'petrol' ? 'selected' : '' ?>>Benzină</option>
              <option value="diesel" <?= ($formData['fuel_type'] ?? '') == 'diesel' ? 'selected' : '' ?>>Motorină</option>
        ...
        <hr>
        <a href="<?= BASE_URL ?>vehicles/view?id=<?= $vehicle['id'] ?>" class="btn btn-secondary">Anulează</a>
